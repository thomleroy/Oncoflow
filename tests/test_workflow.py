...
def test_checklist_update_endpoint():
    _, dossier = create_patient_and_dossier()
    dossier_id = dossier["id"]

    update = client.post(
        f"/dossiers/{dossier_id}/checklists",
        json={"flags": {"identity_validated": True}},
        headers=auth_headers(),
    )
    assert update.status_code == 200
    assert update.json()["checklists"]["identity_validated"] is True


def test_dossier_detail_exposes_patient_messages_and_history():
    ...
    client.post(
        ...
        json={
            ...
            "texte": "Note clinique",
            "mentions": ["physicien"],
            "pieces_jointes": ["doc.pdf"],
            "visibilite": "equipe",
        },
        headers=auth_headers(),
    )

    detail = client.get(f"/ui/dossiers/{dossier_id}")
    assert detail.status_code == 200
    body = detail.json()
    assert body["patient"]["id"] == patient.id
    assert body["dossier"]["id"] == dossier_id
    assert len(body["messages"]) == 1
    assert body["messages"][0]["texte"] == "Note clinique"
...
def test_board_served_with_prefix_root_path():
    prefixed = TestClient(app, root_path="/oncoflow")
    response = prefixed.get("/oncoflow/board")
    assert response.status_code == 200
    assert "text/html" in response.headers.get("content-type", "")
    assert 'const rootPath = "/oncoflow"' in response.text
    assert 'href="/oncoflow/static/style.css"' in response.text
