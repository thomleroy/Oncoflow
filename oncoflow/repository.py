...
class InMemoryRepository:
    ...
    def update_checklists(
        self, dossier_id: str, flags: Dict[str, bool], acteur: str, role: Role
    ) -> Dossier:
        dossier = self.get_dossier(dossier_id)
        merged = dossier.checklists.model_copy(update=flags)
        dossier.checklists = merged
        notification = Notification(
            type="checklist",
            message=f"{acteur} ({role.value}) met a jour la checklist",
            severity="info",
            dossier_id=dossier_id,
        )
    ...
    def get_dossier_detail(self, dossier_id: str) -> DossierDetail:
        dossier = self.get_dossier(dossier_id)
        patient = self.get_patient(dossier.patient_id)
        messages = self.list_messages(dossier_id)
        history = self.get_history(dossier_id)
...
class SQLiteRepository:
    ...
    def update_checklists(
        ...
        merged = dossier.checklists.model_copy(update=flags)
        self.conn.execute(
            "UPDATE dossiers SET checklists = ? WHERE id = ?",
            (self._json_dump(merged.model_dump()), dossier_id),
        )
        dossier.checklists = merged
        notification = Notification(
            type="checklist",
